...
func (be *RedisBackend) indexLookupJob(ctx context.Context, limit int64, opts *resource.JobListParams) ([]string, error) {
...
func (be *RedisBackend) indexLookupQueue(ctx context.Context, pipe redis.Pipeliner, limit int64, opts *resource.QueueListParams) ([]string, error) {
	var selectors []*redis.StringSliceCmd
	_, err := be.rds.Pipelined(ctx, func(pipe redis.Pipeliner) error {
		selectors = be.indexLookupQueueBySelectors(ctx, pipe, limit, opts)
		return nil
	})
	if err != nil {
		return nil, err
...
func (be *RedisBackend) indexLookupQueueBySelectors(ctx context.Context, pipe redis.Pipeliner, limit int64, opts *resource.QueueListParams) []*redis.StringSliceCmd {
	sel := opts.Selectors
	if sel.Len() == 0 {
		return nil
	}
	...
	if len(sel.NotNames) > 0 {
		// for _, notname := range sel.NotNames {
		// 	// pipe.S
		// }
	}
	if len(sel.Names) > 0 {
		// pipe.SUnionStore(ctx, dest, sel.Names...)
	}
	if len(sel.In) > 0 {
		// pipe.SUnionStore(ctx, dest, append(sel.In, dest)...)
	}
	return nil
}
