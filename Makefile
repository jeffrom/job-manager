...
.PHONY: test.race
test.race: gen $(gofiles) | $(staticcheck) $(buf)
	GO111MODULE=on go test -race ./...
	cd mjob && GO111MODULE=on go test -race ./...

.PHONY: bench
bench: gen $(gofiles) | $(staticcheck) $(buf)
	GO111MODULE=on go test -run ^$$ -bench . -benchmem -benchtime 4s ./...
	cd mjob && GO111MODULE=on go test  -run ^$$ -bench . -benchmem -benchtime 4s ./...

.PHONY: bench.race
bench.race: gen $(gofiles) | $(staticcheck) $(buf)
	GO111MODULE=on go test -run ^$$ -bench . -benchmem -race ./...
	cd mjob && GO111MODULE=on go test  -run ^$$ -bench . -benchmem -race ./...
